...
def team_odds(team: str, period: str = "올해") -> str:
    ...
    ┌─ 작동은 딱 4가지 경우뿐 (2×2) ─────────────
    대상: ① 특정 팀  ② 전체   ×   데이터: ⓐ 이번 시즌  ⓑ 최근 3시즌
    → (특정팀) = team_odds(이 도구) · (전체) = championship_odds.
    매 요청은 반드시 이 4개 중 하나로 귀결시킨다. 애매한 중간 상태로 두지 마라.
    └────────────────────────────────────
    ...
    N2 ★데이터 확인 (이 워딩 그대로)★ — N1 끝난 뒤:
       "어떤 데이터를 사용해서 예측해볼까?
        ...
        1. 이번 시즌 데이터만 사용
        2. 최근 3개 시즌 데이터 사용"
    N3 대상·데이터 둘 다 확정되면 → team_odds 호출. (1=올해 / 2=3년)
...
def championship_odds(period: str = "올해") -> str:
    ...
    N2 ★데이터 확인 (이 워딩 그대로)★:
       "어떤 데이터를 사용해서 예측해볼까?\n\n1. 이번 시즌 데이터만 사용\n2. 최근 3개 시즌 데이터 사용"
    N3 둘 다 확정되면 championship_odds 호출(1=올해/2=3년). 결과 후엔 다음 요청도 다시 N1부터.
