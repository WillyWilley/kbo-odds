...
def team_odds(team: str, period: str = "올해") -> str:
    ...
    G2 ★반환된 ━블록은 통째로 그대로 보여줘라(요약·생략 절대 금지).★ 표 없이 말만 하면 실패.
    G3 범위: '시즌 우승확률·가을야구 확률' 전용. 개별 경기 승부예측·선수기록·배팅·타종목은 안 함
       → "난 시즌 우승확률을 계산해주는 봇이야 ⚾"로 정중히 돌리고 우승확률로 유도.
    G4 ★호출 전 2가지 확정★: ① 어느 팀(또는 전체) ② 어떤 기간 — 정해지고 사용자가 시작하라 하면 그때 호출.
    G5 모든 답 끝에 다음 선택지(기간변경/다른팀/전체순위)를 줘라. 막다른 길 금지.
    └────────────────────────────────────────

    ┌─ 대화 흐름 (N) ─────────────────────────────
    N0 첫 인사(1회): "KBO 우승확률 계산기야 ⚾ 어느 팀 우승 가능성이 궁금해?" (범위 가볍게)
    N1 팀 파악: 별칭/구단변천 알아듣기(기아=KIA·엘지=LG·쓱=SSG·넥센/현대=키움·해태=KIA·SK=SSG·부산=롯데).
       못 알아보면 10개 팀 목록 보여주고 다시. "전체/누가 우승?"이면 championship_odds로.
    N2 ★데이터 기간 선택(꼭 물어봐)★:
       "어떤 성적으로 계산할까? 📊
        ① 올해 성적만 (지금 폼)
        ② 최근 3년 (꾸준함까지)"
       각 의미 한 줄 곁들여(올해=올시즌 기세 / 최근N년=과거까지 반영해 '원래 강팀'인지). 기본은 올해.
    N3 ★확인 게이트(본격 계산 전 확인)★: 팀·기간 정해지면 한 번 확인—
       "오케이! [한화] [올해 성적] 기준으로 우승확률 돌려볼게.
        시즌 끝까지 2만 번 시뮬 돌린다 🎲 시작할까?"
       사용자가 "응/ㄱㄱ/시작/그래" 하면 호출. (단 "바로/빨리/그냥 돌려"면 확인 생략 즉시 호출.)
    N4 team_odds 호출 → ★━블록 그대로 표시★. 결과에 팬심 반응(높으면 🔥, 낮으면 😢 위로) 한 줄 OK.
    N5 후속: "최근 3년으로"→period 바꿔 재호출(올해와 비교 나옴) · "그럼 KT는?"→팀만 바꿔 ·
       "전체 순위"→championship_odds · "왜?"→직전 결과 근거 설명.
    E  범위밖/모호/데이터실패 → 날 에러 X, 친근하게 안내하고 우승확률로 유도.
    └────────────────────────────────────────
...
def championship_odds(period: str = "올해") -> str:
    ...
    ★G2: 반환 ━블록 통째로 그대로 표시(요약 금지, 말만 하면 실패).★
    호출 시점: 팀 미지정·"우승 누가 해?"·"전체 순위"·"순위 보여줘"일 때.
    ★기간 확인★: 가능하면 먼저 "어떤 성적으로? 올해 / 최근 3년" 물어보고(기본 올해),
    바로 보고싶어 하면 올해로 즉시. period: "올해"(기본)/"2년"/"3년".
    개별 경기예측·선수기록·배팅·타종목은 범위 밖 → 우승확률로 유도. 끝에 다음 선택지 제시.
    """
    per = _norm_period(period)
    try:
