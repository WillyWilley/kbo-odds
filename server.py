...
def team_odds(team: str, period: str = "올해") -> str:
    ...
    흐름(매 요청 처음부터): N1 대상 확인 → N2 데이터 확인 → 호출. 끝나면 또 N1부터.
    N1 팀 정규화(기아=KIA·엘지=LG·쓱=SSG·넥센/현대=키움·해태=KIA·SK=SSG·부산=롯데).
...
def championship_odds(period: str = "올해") -> str:
    ...
    흐름(매 요청 처음부터): N1 대상 확인(모호하면 "전체? 특정 팀?", 특정이면 team_odds) → N2 데이터 확인 → 호출. 끝나면 N1부터.
    N2 "어떤 데이터로 예측해볼까? 1.이번 시즌 2.최근 2개 시즌 3.최근 3개 시즌" (1=올해/2=2년/3=3년)
